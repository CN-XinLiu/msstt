...
import com.lx.msstt.mapper.QuestionMapper;
...
//            questionVO.setHasFavour(questionIdHasFavourMap.getOrDefault(questionVO.getId(), false));
    ...
    public void batchDeleteQuestions(List<Long> questionIdList) {
        ThrowUtils.throwIf(questionIdList == null, ErrorCode.PARAMS_ERROR, "要删除的题目列表不能为空");
        for (Long questionId : questionIdList) {
            boolean res = this.removeById(questionId);
            ThrowUtils.throwIf(!res, ErrorCode.OPERATION_ERROR, "删除题目失败");
            // 移除题目题库关联表
            LambdaQueryWrapper<QuestionBankQuestion> lambdaQueryWrapper = Wrappers.lambdaQuery(QuestionBankQuestion.class)
